...
function mod:CreateIcon(frame, index)
	...
	stackText:SetShadowOffset(1,0)
	stackText:SetPoint("BOTTOMRIGHT", icon, -7, 5)
	stackText:SetJustifyH("LEFT")
	stackText:SetJustifyV("BOTTOM")
	icon.stackText = stackText
	
	local durationFrame = CreateFrame("Frame", nil, icon)
	durationFrame:SetFrameLevel(cooldown:GetFrameLevel()+1)
	durationFrame:SetAllPoints(icon)
	local durationText = durationFrame:CreateFontString(nil, "OVERLAY")
	durationText:SetFont(font, GridFrame.db.profile.fontSize, "OUTLINE")
	durationText:SetShadowColor(0,0,0,1)
	durationText:SetShadowOffset(1,0)
	durationText:SetPoint("BOTTOMRIGHT", icon, 4, -2)
	durationText:SetJustifyH("RIGHT")
	durationText:SetJustifyV("BOTTOM")
...
function mod.UpdateOptionsForIndicator(GridFrame, indicator, name, order)
	...
	if index > mod.db.profile.count then
		Grid.options.args["GridIndicator"].args[indicator] = nil
	end
end
...
function mod.framePrototype.SetIndicator(frame, indicator, color, text, value, maxValue, texture, start, duration, stack)
	...
	if expiration > GetTime() and math.floor(expiration-GetTime()) < 10 then
		icon.durationText:SetText(math.floor(expiration-GetTime()))
