...
local function SetButtonStatus(info, value)
	if value then
		...
		addon.db.profile.order[index] = button:GetName()
...
local function OrderSort(a, b)
	return (defaultOrder[a] or #defaultOrder + 1) < (defaultOrder[b] or #defaultOrder + 1)
end
...
------------------------------------------------------------------------------]]
do
	local generalTab = options.args.generalTab.args
	for index = 1, #defaultOrder do
		local button = _G[defaultOrder[index]]
		if button then
			local text
			if button.tooltipText then
				text = button.tooltipText:gsub("%s*|c%x%x%x%x%x%x%x%x%(%S+%)|r$", "")
			else
				text = button:GetName():gsub("MicroButton$", "")
			end
			generalTab[defaultOrder[index]] = {
				type = 'toggle',
				order = index + 5,
				name = text,
				desc = L["Show the %s button."]:format(text)
			}
		end
	end

	local option = generalTab['GuildMicroButton']								-- Text changes so use most generic
	if option then
		option.name = GUILD
		option.desc = L["Show the %s button."]:format(GUILD)
	end
end
...
elseif not addon.platform then
	local strataLayers = LibStub('LibDisplayAssist-1.3').StrataLayers
